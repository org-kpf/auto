class osd():
    def __init__(self,id,name,data_dir,status,action_status,up,IN,host_id,host_name,pool_id,pool_name,disk_id,disk_device,partition_id,partition_path,osd_id,create):
        ...
        self.osd_id = osd_id
        self.create = create
    def create_osd(self,disk,cache_partition,read_cache_size,role):
        ...
        if cache_partition == '' and read_cache_size != '':
            ...
            cmd = 'osd create --disk %d --cache-partition %s --read-cache-size %s --role %s' % (disk, cache_partition, read_cache_size, role)
        return cmd
