...
class volume():
    def __init__(self,id,name,size,pool_id,pool_name,snapshot_id,snapshot_name,status,action_status,flattened,format,
                 qos_max_total_iops,qos_max_total_bw,qos_burst_total_iops,qos_burst_total_bw,description,create,sn,performance_priority,qos_enabled,cluster):
        self.id = id
        self.name = name
        self.size = size
    ...
    def create_volume(self,pool,size,name,format,performance_priority,qos_enabled,max_total_iops,max_total_bw,max_burst_iops,max_burst_bw):
        '''
    ...
    def CreateSnapshot(self,name,block_volume,description='',check_times=0,check_interval=0):
        ...
        for i in range(0, check_times):
            ...
            if result == 'active':
                ...
                volume_id = int(out[6])
                volume_name = out[7]
                return snapshot.snapshot(id,name,snap_name,size,allocated_size,status,volume_id,volume_name,self.cluster)
            if result != 'active':
