...
import socket
class pool():
    def __init__(self,id,name,protection_domain_id,replicate_size,pool_type,failure_domain_type,
                 device_type,status,action_status,pool_mode,pool_role,pool_cluster='cluster1'):
        self.id = id
        self.name = name
    ...
    def create_pool(self,osds,pool_type,size,pool_role,failure_domain_type,pool_name,data_chunk_num,coding_chunk_num):
        '''
        osds，必选，osd的列表，可以通过show osd查看到，例子：1,2,3,4,5,6,7,8
        pool-type 必选，存储池的类型，取值范围（replicated和erasure）二选一，例子：replicated
        size 可选，如果pool_type选副本，则必选，取值范围1-6，例子：3
        ...
        data-chunk-num  可选，如果pool_type选erasure，则必选，取值范围：正整数
        coding-chunk-num 可选，如果pool_type选erasure，则必选，取值范围：正整数
        '''
...
#     pool_role = 'data'
    ...
    # POOL1 =  pool(id,name,protection_domain_id,replicate_size,pool_type,failure_domain_type,device_type,status,action_status,pool_mode,pool_role)
    # print(POOL1.id)
    # print(type(POOL1))
