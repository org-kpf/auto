...
class EBS(object):
    ...
    def ebs_connect(self, ip, user, password, port=22):
        ...
        return host(id, name, type, roles, public_ips, private_ip, admin_ip, gateway_ips, description, protection_domain_id, status, up, cluster)
    def cmd(self,cmd):
        ...
        for i in range(0, check_times):
            ...
            if result_status == 'active':
                ...
                pool_role = self.cmd('''xms-cli -f '{{range .}}{{println .pool_role}}{{end}}' --user %s --password %s pool list -q "name.raw:%s"'''
                                     % (self.admin_user, self.admin_password, name))
                return pool(id, name, protection_domain_id, replicate_size, type, failure_domain_type, device_type, status, action_status, pool_mode, pool_role, self.cluster)
        ...
        for i in range(0, check_times):
            ...
            if result == 'active':
                ...
                return volume(id, name, size, pool_id, pool_name, snapshot_id, snapshot_name, status, action_status,
                              ...
                              qos_burst_total_bw,description, create, sn, performance_priority, qos_enabled, cluster)
            elif result != 'active':
        ...
        for i in range(0, check_times):
            ...
            if result == 'active':
                ...
                up = self.cmd('''xms-cli -f '{{range .}}{{println .up}}{{end}}' --user %s --password %s host list -q "admin_ip:%s"'''
                                % (self.admin_user, self.admin_password, admin_ip))
                return host(int(id), name, type, roles, public_ips, private_ip, admin_ip, gateway_ips, description, protection_domain_id, 'active', up, self.cluster)
            elif result != 'active':
    ...
    def ShowHost(self, admin_ip, check_times=1, check_interval=1):
        ...
        for i in range(0, check_times):
            ...
            if result_status == 'active':
                ...
                up = self.cmd('''xms-cli -f '{{range .}}{{println .up}}{{end}}' --user %s --password %s host list -q "admin_ip:%s"'''
                                % (self.admin_user, self.admin_password, admin_ip))
                return host(id, name, type, roles, public_ips, private_ip, admin_ip, gateway_ips, description, protection_domain_id, status, up, self.cluster)
            if i == check_times:
    ...
    def CreateOsd(self, disk_list, cache_partition_list=[], read_cache_size=268435456, role='data', check_times=20, check_interval=10):
        ...
        osd_list1 = []
        osd_list2 = []
        print('disk_list', osd_list1)
        print('disk_list-1', len(disk_list))
        ...
        if 'error' in result:
            ...
            for i in range(0, check_times):
                ...
                if result_status != 'active':
                    ...
                    return client_group(id, name, type, description, client_num, access_path_num, block_volume_num, result_status, codes, self.cluster)
                if i == check_times:
    ...
    def ShowClientGroup(self, name, check_times=1, check_interval=1):
        ...
        for i in range(0,check_times):
            ...
            if result_status == 'active':
                ...
                cluster = self.cluster
                return client_group(id, name, type, description, client_num, access_path_num, block_volume_num, status, codes, cluster)
            if i == check_times:
        ...
        for i in range(0, check_times):
            ...
            if result_status == 'active':
                ...
                try:
                    ...
                    client_group_id = ''
                    client_group_name = ''
                return mapping_group(id, access_path_id, access_path_name, client_group_id, client_group_name, status, create_time, self.cluster)
            elif result_status != 'active':
                time.sleep(check_interval)
    ...
    def CreateTarget(self,access_path,host,check_times=0,check_interval=0):
        ...
        for i in range(0, check_times):
            ...
            if result_status == 'active':
                ...
                status = out[6]
                create = out[7]
                return target(id, host_id,board,port,iqn,access_path_id,status,create,self.cluster)
            elif result_status != 'active':
                time.sleep(check_interval)
                print('第%d次检查，检查间隔%d秒，创建映射组失败，状态是%s' % (i, check_interval, result_status))
                i += 1
    ...
    def ShowTarget(self,access_path_name):
        ...
        for k in target_id_list:
            ...
            status = result[6]
            create = result[7]
            target_obj = target(id, host_id, board, port, iqn, access_path_id, status, create, self.cluster)
